...
export async function requireMediaAdmin(request?: Request) {
  ...
  const user = { _id: new Types.ObjectId(session.user.id), role: session.user.role };
  ...
  if (request && !await withinRateLimit("media:" + user._id.toString(), 180, 60000)) throw new MediaApiError(429, "RATE_LIMITED");
  return user;
}
...
export function apiFailure(error: unknown): Response {
  if (error instanceof AuthError) return json({ error: error.code === "PASSWORD_CHANGE_REQUIRED" ? "FORBIDDEN" : error.code }, error.status);
