...
export function PageHero({ eyebrow, title, body, image, ctaLabel, ctaHref, breadcrumbs }: {
  ...
  return (
    <section className="relative min-h-[58vh] overflow-hidden bg-charcoal text-ivory stone-vein">
      {image ? (
        <CloudinaryImage media={image} priority sizes="100vw" className="absolute inset-0 h-full w-full object-cover opacity-70" />
      ) : <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(156,127,69,.35),transparent_34%),linear-gradient(135deg,#131313,#000)]" />}
      <div className="absolute inset-0 bg-[linear-gradient(0deg,rgba(19,19,19,.96),rgba(19,19,19,.45)_55%,rgba(19,19,19,.2)),linear-gradient(90deg,rgba(0,0,0,.78),rgba(0,0,0,.18))]" />
      <div className="page-shell relative flex min-h-[58vh] flex-col justify-end pb-16 pt-32">
        ...
        {eyebrow ? <div className="mb-5 flex items-center gap-4"><span className="gold-rule" aria-hidden="true" /><p className="editorial-label text-accent">{eyebrow}</p></div> : null}
        <h1 className="max-w-5xl font-heading text-display uppercase">{title}</h1>
        {body ? <p className="mt-6 max-w-2xl text-body leading-8 text-ivory/80">{body}</p> : null}
        {ctaLabel && ctaHref ? (
          <Link href={ctaHref as Route} className="mt-8 inline-flex w-fit items-center border border-accent px-6 py-3 text-sm font-semibold uppercase tracking-[0.18em] text-ivory transition hover:bg-accent hover:text-charcoal">
            {ctaLabel}
          </Link>
        ) : null}
      </div>
