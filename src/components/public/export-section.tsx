...
export function ExportSection({ section, whatsappHref }: { section: PublicSection; whatsappHref?: string }) {
  return <section className="py-section"><div className="page-shell grid items-center gap-12 lg:grid-cols-2">
    ...
    <Reveal><SectionHeading eyebrow={section.eyebrowLabel} heading={section.heading} subheading={section.subheading} />{section.bodyText && <div className="prose prose-stone" dangerouslySetInnerHTML={{ __html: section.bodyText }} />}
      ...
      {section.ctaLabel && (whatsappHref ?? section.ctaUrl) && <a href={whatsappHref ?? section.ctaUrl} target={whatsappHref ? "_blank" : undefined} rel={whatsappHref ? "noopener noreferrer" : undefined} className="mt-9 inline-flex min-h-12 items-center bg-accent px-6 text-charcoal">{section.ctaLabel}</a>}
    </Reveal>
  </div></section>;
}
