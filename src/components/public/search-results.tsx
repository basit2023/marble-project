  ...
  return (
    <section className="page-shell py-section">
      <form className="mb-8 flex flex-col gap-3 sm:flex-row" action="/search">
        <input name="q" defaultValue={query} placeholder="Search products, projects and articles" className="min-w-0 flex-1 border border-ivory/25 bg-transparent px-4 py-3 placeholder:text-ivory/40" />
        {activeTypes.map((type) => <input key={type} type="hidden" name="type" value={type} />)}
        <button className="bg-accent px-6 py-3 text-sm font-semibold uppercase tracking-[0.18em] text-charcoal">Search</button>
      </form>
      <div className="mb-10 flex flex-wrap gap-3">
        {(["Product", "Project", "Blog"] as const).map((type) => <button key={type} type="button" onClick={() => toggleType(type)} className={`border px-4 py-2 text-sm ${activeTypes.includes(type) ? "border-accent text-accent" : "border-ivory/25"}`}>{type}</button>)}
      </div>
      <div className="grid gap-6">
        {results.map((result) => (
          <Link href={result.url as Route} key={`${result.type}-${result.id}`} className="grid gap-5 border-t border-ivory/10 pt-6 sm:grid-cols-[10rem_1fr]">
            <div className="aspect-[4/3] bg-white/10">{result.image ? <CloudinaryImage media={result.image} sizes="160px" className="h-full w-full object-cover" /> : null}</div>
