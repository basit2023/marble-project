  ...
  return (
    <form onSubmit={submit} className="page-shell py-section">
      <ol className="mb-10 grid grid-cols-4 gap-2 text-xs uppercase tracking-[0.12em] text-muted">
        {steps.map((label, index) => <li key={label} className={`border-t pt-3 ${index <= step ? "border-accent text-charcoal" : "border-charcoal/20"}`}>{label}</li>)}
      </ol>
      {step === 0 ? (
        <div className="grid gap-4 md:grid-cols-2">
          <select {...form.register("projectType")} className="border border-charcoal/20 bg-ivory px-4 py-3"><option value="">Project type</option><option>Residential</option><option>Commercial</option><option>Export container</option><option>Hospitality</option></select>
          <select {...form.register("timeline")} className="border border-charcoal/20 bg-ivory px-4 py-3"><option value="">Timeline</option><option>Urgent</option><option>1-3 months</option><option>3-6 months</option><option>Planning stage</option></select>
          <input {...form.register("quantity")} placeholder="Quantity" className="border border-charcoal/20 bg-transparent px-4 py-3" />
          <input {...form.register("unit")} placeholder="Unit, e.g. sqft, sqm, container" className="border border-charcoal/20 bg-transparent px-4 py-3" />
        </div>
      ) : null}
      {step === 1 ? (
        <div className="grid gap-8">
          {Object.entries(grouped).map(([category, items]) => (
            <fieldset key={category} className="border-t border-charcoal/10 pt-5">
              <legend className="font-heading text-3xl">{category}</legend>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((product) => <label key={product.id} className="flex gap-3 border border-charcoal/15 p-3"><input type="checkbox" value={product.id} {...form.register("productInterest")} />{product.name}</label>)}
              </div>
            </fieldset>
          ))}
        </div>
      ...
      {step === 2 ? (
        <div className="grid gap-4">
          <div className="grid gap-4 md:grid-cols-2">
            <input {...form.register("name")} placeholder="Name" className="border border-charcoal/20 bg-transparent px-4 py-3" />
            <input {...form.register("email")} type="email" placeholder="Email" className="border border-charcoal/20 bg-transparent px-4 py-3" />
            <input {...form.register("phone")} placeholder="Phone / WhatsApp" className="border border-charcoal/20 bg-transparent px-4 py-3" />
            <input {...form.register("country")} placeholder="Project location / country" className="border border-charcoal/20 bg-transparent px-4 py-3" />
          </div>
          <textarea {...form.register("message")} rows={5} placeholder="Project notes" className="border border-charcoal/20 bg-transparent px-4 py-3" />
      ...
      {step === 3 ? (
        <div className="grid gap-4 border border-charcoal/15 p-6">
      ...
      {Object.keys(form.formState.errors).length ? <p className="mt-6 text-sm text-red-700">Please complete the required fields for this step.</p> : null}
      {status ? <p className="mt-6 text-sm text-green-800">{status}</p> : null}
      <div className="mt-8 flex gap-3">
        {step > 0 ? <button type="button" onClick={() => setStep((value) => value - 1)} className="border border-charcoal px-6 py-3 text-sm uppercase tracking-[0.18em]">Back</button> : null}
        {step < steps.length - 1 ? <button type="button" onClick={next} className="bg-charcoal px-6 py-3 text-sm uppercase tracking-[0.18em] text-ivory">Next</button> : <button disabled={pending} className="bg-charcoal px-6 py-3 text-sm uppercase tracking-[0.18em] text-ivory disabled:opacity-60">Submit quote</button>}
      </div>
    </form>
  );
}
