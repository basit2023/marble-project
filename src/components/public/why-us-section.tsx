...
export function WhyUsSection({ section }: { section: PublicSection }) {
  return <section className="py-section"><div className="page-shell"><Reveal><SectionHeading eyebrow={section.eyebrowLabel} heading={section.heading} subheading={section.subheading} /></Reveal>
    <div className="grid gap-px bg-black/10 sm:grid-cols-2 xl:grid-cols-4">{section.items.map((item) => { const Icon = icons[item.iconKey ?? "quality"] ?? Sparkles; return <Reveal key={item.key} className="bg-ivory p-8"><Icon aria-hidden="true" className="mb-8 size-8 text-accent" /><h3 className="font-heading text-2xl">{item.title}</h3>{item.body && <p className="mt-4 leading-7 text-muted">{item.body}</p>}</Reveal>; })}</div>
  </div></section>;
}
