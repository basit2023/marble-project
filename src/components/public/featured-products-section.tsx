...
export function FeaturedProductsSection({ section, products, copy }: { section: PublicSection; products: PublicProduct[]; copy: PublicCopy }) {
  ...
  return <section className="overflow-hidden py-section"><div className="page-shell"><div className="flex items-end justify-between gap-6"><SectionHeading eyebrow={section.eyebrowLabel} heading={section.heading} subheading={section.subheading} /><div className="mb-10 hidden gap-2 sm:flex"><button onClick={() => move(-1)} aria-label={copy.labels.previousProducts} className="grid size-11 place-items-center border border-charcoal"><ChevronLeft /></button><button onClick={() => move(1)} aria-label={copy.labels.nextProducts} className="grid size-11 place-items-center border border-charcoal"><ChevronRight /></button></div></div>
