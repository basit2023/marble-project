...
export function FilterableProjectGrid({ projects }: { projects: ProjectCardDTO[] }) {
  ...
  return (
    <section className="page-shell py-section">
      <div className="stone-panel mb-10 grid gap-px bg-white/10 p-px sm:grid-cols-2">
        <select aria-label="Filter by project type" value={type} onChange={(event) => setType(event.target.value)} className="border border-white/15 bg-black px-3 py-3 text-ivory">
          <option value="">All project types</option>
          {options.types.map((option) => <option key={option}>{option}</option>)}
        </select>
        <select aria-label="Filter by country" value={country} onChange={(event) => setCountry(event.target.value)} className="border border-white/15 bg-black px-3 py-3 text-ivory">
