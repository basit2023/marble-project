...
# International format, digits only (country code included).
...
```
{
  ...
  "packages": {
    ...
    "node_modules/cloudinary": {
      "version": "2.11.0",
      "resolved": "https://registry.npmjs.org/cloudinary/-/cloudinary-2.11.0.tgz",
      "integrity": "sha512-o2pAkDDqrzPFE/mn9GCfFjBIGQmPs++Ts6rtkFl4vY528DNHiMovKWjH16HBjeGa9iwI7eAad24aWN2emT0aww==",
      "license": "MIT",
      "dependencies": {
        "lodash": "^4.17.23"
      },
...
try {
  for (const [sortOrder, record] of records.entries()) {
    ...
    await SystemContent.updateOne({ key: copy.key }, { $setOnInsert: {
      ...copy, sortOrder, isActive: true, isDeleted: false,
      createdBy: "system:foundation-seed", updatedBy: "system:foundation-seed",
...
## src/app/(admin)/admin/[...unmatched]/page.tsx

```
import { notFound } from "next/navigation";
export default function UnmatchedAdminPage() { notFound(); }
...
export default function ErrorBoundary({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return <SystemScreen screen="error" reset={reset} />;
}
...
@theme {
  ...
  --color-accent: #80633a;
  --color-muted: #62615a;
  --text-caption: clamp(0.75rem, 0.72rem + 0.12vw, 0.875rem);
  --text-body: clamp(1rem, 0.96rem + 0.2vw, 1.125rem);
  --text-title: clamp(2rem, 1.35rem + 2.8vw, 4.5rem);
  --text-display: clamp(2.75rem, 1.5rem + 5vw, 7rem);
  --text-display--line-height: 1.04;
  --text-title--line-height: 1.1;
  --spacing: 0.25rem;
  --spacing-section: clamp(4rem, 2rem + 6vw, 9rem);
  --spacing-gutter: clamp(1.25rem, 0.6rem + 3vw, 4rem);
  --container-editorial: 90rem;
...
@layer base {
  body { @apply bg-ivory text-charcoal font-sans antialiased; }
  :focus-visible { outline: 2px solid var(--color-accent); outline-offset: 4px; }
  button, a, input, select, textarea { -webkit-tap-highlight-color: transparent; }
  button, a { touch-action: manipulation; }
  button:not(:disabled) { cursor: pointer; }
  ::selection { background: var(--color-charcoal); color: var(--color-ivory); }
}
@utility page-shell {
  width: 100%;
...
@utility editorial-label { font-size: var(--text-caption); letter-spacing: 0.18em; text-transform: uppercase; }
...
export function Modal({ open, onClose, title, closeLabel, children }: {
  ...
  return <dialog ref={ref} onCancel={(event) => { event.preventDefault(); onClose(); }} onClose={onClose}
    aria-labelledby={titleId} className="fixed inset-0 m-auto max-h-[90dvh] w-[calc(100%-2.5rem)] max-w-xl overflow-auto bg-ivory p-6 text-charcoal backdrop:bg-black/60">
...
## src/components/ui/system-screens.tsx
...
export function SystemScreen({ screen, reset }: { screen: ScreenKey; reset?: () => void }) {
  const copy = useContext(CopyContext)[screen];
  if (!copy) return <main className="page-shell min-h-96" />;
  return <main className="page-shell flex min-h-[65vh] flex-col justify-center gap-6 py-16" aria-busy={screen === "loading"}>
    ...
    {reset && <button type="button" onClick={reset} className="w-fit border border-charcoal px-6 py-3">{copy.actionLabel}</button>}
...
export const getSystemCopy = cache(async (): Promise<SystemCopy> => {
  try {
    await connectDB();
    const records = await SystemContent.find(publicFilter).sort({ sortOrder: 1 }).lean().exec();
...
export function buildCsp(nonce?: string): string {
  ...
  return [
    ...
    `connect-src 'self' https://res.cloudinary.com${development ? " ws: wss:" : ""}`,
    "object-src 'none'",
    "base-uri 'self'",
    "form-action 'self'",
...
import { contentFields } from "@/models/content-fields";
...
type SystemContentDocument = InferSchemaType<typeof schema>;
export const SystemContent = (models.SystemContent as Model<SystemContentDocument> | undefined)
  ?? model<SystemContentDocument>("SystemContent", schema);
